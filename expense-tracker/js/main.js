...
document.getElementById("saveTransactionBtn")?.addEventListener("click", async () => {
    ...
    if (!payload.expense_date || isNaN(payload.amount) || payload.amount <= 0) {
        Swal.fire("กรุณากรอกข้อมูล", "", "warning");
        return;
    }
    showLoading();
    try {
...
async function loadReports() {
    ...
    try {
        ...
        if (!cats.labels?.length) topDiv.innerHTML = '<div class="text-center text-slate-400">ไม่มีข้อมูล</div>';
        else {
...
async function loadBudgetPage() {
    ...
    try {
        ...
        if (budgetTHB > 0 && expenseTHB > budgetTHB) {
            ...
            alertDiv.innerHTML = `<i class="fas fa-exclamation-triangle mr-2"></i> เกินงบประมาณ! ใช้ไป ${expenseTHB.toFixed(2)} / ${budgetTHB.toFixed(2)} บาท`;
...
function initSelects() {
    ...
    document.getElementById("userName").innerText = "ผู้ใช้";
}
initSelects();
loadHome();
