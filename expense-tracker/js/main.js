...
document.getElementById("saveTransactionBtn")?.addEventListener("click", async () => {
    ...
    if(!payload.expense_date || isNaN(payload.amount) || payload.amount<=0) { Swal.fire("กรุณากรอกข้อมูล","","warning"); return; }
    showLoading();
    try { await addExpense(payload); Swal.fire("บันทึกสำเร็จ!","","success"); document.getElementById("amount").value=""; document.getElementById("description").value=""; switchPage("home"); loadHome(); } catch(e) { Swal.fire("Error",e.message,"error"); }
    hideLoading();
});
...
async function loadReports() {
    ...
    document.getElementById("topCategories").innerHTML = !sorted.length ? '<div class="text-center text-slate-400">ไม่มีข้อมูล</div>' : sorted.map(c => `<div class="flex justify-between items-center py-2 border-b border-slate-700"><span class="text-white">${c.label}</span><span class="text-indigo-400 font-semibold">${c.value.toFixed(2)} บาท</span></div>`).join("");
    hideLoading();
}
async function loadBudgetPage() {
    ...
    if(budget>0 && expense>budget) { alertDiv.classList.remove("hidden"); alertDiv.innerHTML = `<i class="fas fa-exclamation-triangle mr-2"></i> เกินงบประมาณ! ใช้ไป ${expense.toFixed(2)} / ${budget.toFixed(2)} บาท`; }
...
document.getElementById("saveBudgetBtn")?.addEventListener("click", async () => {
    ...
    showLoading();
    try { await setBudgets(currentMonth,currentYear,budgets); Swal.fire("บันทึกงบประมาณแล้ว","","success"); loadBudgetPage(); } catch(e) { Swal.fire("Error",e.message,"error"); }
    hideLoading();
});
